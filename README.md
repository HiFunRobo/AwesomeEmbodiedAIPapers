...
### 合并已有 Markdown 并排序（默认行为）

若输出文件已存在，**默认**会与已有表格合并（同 arXiv ID 或同 GitHub 仓库以本次结果为准），并按 Year 排序。

```bash
# 默认即合并；无需再写 --merge
python paper_collection_agent.py https://arxiv.org/abs/2401.08399 -o ref.md

# 时间降序（新 → 旧）
python paper_collection_agent.py https://arxiv.org/abs/2401.08399 -o ref.md --desc

# 不合并：直接覆盖输出文件，仅保留本次收集的条目
python paper_collection_agent.py https://arxiv.org/abs/2401.08399 -o ref.md --no-merge
```
...
| `--merge` / `--no-merge` | **默认 `--merge`（合并）**：`-o` 已存在且可读时与旧表合并去重后排序；**`--no-merge`** 则直接覆盖，仅本次条目 |
| `--desc` | 按 Year 降序；默认升序 |
